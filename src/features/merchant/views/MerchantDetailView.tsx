...
export function MerchantDetailView({ merchantId }: { merchantId: string }) {
  ...
  if (query.isLoading) return <Page title="商户详情" onBack={() => router.history.back()}><LoadingState /></Page>;
  if (query.isError || !query.data) return <Page title="商户详情" onBack={() => router.history.back()}><ErrorState title="加载失败" onRetry={() => query.refetch()} /></Page>;
